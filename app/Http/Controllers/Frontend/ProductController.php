        ...
        if ($slug !=  '' && $sku != '') {
            ...
            if ($product_stock) {
                ...
                $response = [
                    ...
                    'name' => optional(optional($product_stock->product))->getTranslation('name', $lang) ?? '',
                    'slug' => optional($product_stock->product)->slug ?? '',
                    'product_type' => optional($product_stock->product)->product_type ?? '',
                    'brand' => optional(optional($product_stock->product)->brand)->getTranslation('name', $lang) ?? '',
                    'category' => $category,
                    'video_provider' => optional($product_stock->product)->video_provider ?? '',
                    'video_link' => optional($product_stock->product)->video_link ?? '',
                    'return_refund' => optional($product_stock->product)->return_refund ?? '',
                    'published' => optional($product_stock->product)->published ?? '',
                    'photos' => $photos,
                    'thumbnail_image' => get_product_image(optional($product_stock->product)->thumbnail_img ?? ''),
                    'variant_image' => $product_stock->image != null ? get_product_image($product_stock->image) : '',
                    'tags' => explode(',', optional(optional($product_stock->product))->getTranslation('tags', $lang) ?? ''),
                    'status' => $product_stock->status,
                    'sku' => $product_stock->sku ?? '',
                    'quantity' => $product_stock->qty ?? 0,
                    'description' => optional(optional($product_stock->product))->getTranslation('description', $lang) ?? '',
                    'short_description' => optional(optional($product_stock->product))->getTranslation('short_description', $lang) ?? '',
                    'stroked_price' => $priceData['original_price'] ?? 0,
                    'main_price' => $priceData['discounted_price'] ?? 0,
                    'offer_tag' => $priceData['offer_tag'] ?? '',
                    'current_stock' => (int) $product_stock->qty,
                    'rating' => (float) (optional($product_stock->product)->rating ?? 0),
                    'rating_count' => (int) Review::where(['product_id' => $product_stock->product_id])->count(),
                    'tabs' => optional($product_stock->product)->tabsLang ?? [],
                    ...
                    'meta_keywords' => optional(optional($product_stock->product))->getSeoTranslation('meta_keywords', $lang) ?? '',
