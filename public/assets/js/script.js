(function ($) {
	...
	if ($(".preloader-close").length) {
		$(".preloader-close").on("click", function () {
			$('.loader-wrap').delay(30).fadeOut(100);
		})
	}
	...
	// Removed: Dynamic dropdown buttons - now using inline arrows
	...
	if ($('.mobile-menu').length) {
		...
		$(document).on('click', '.mobile-menu a:not(.dropdown-btn span)', function (e) {
	...
	if ($('.category-menu li.category-dropdown ul').length) {
		$('.category-menu .category-list li.category-dropdown').append('<div class="dropdown-btn"><span class="far fa-plus"></span></div>');
	...
	if ($('.popular-carousel').length) {
		$('.popular-carousel').owlCarousel({
			...
			smartSpeed: 500,
			autoplay: 1000,
			navText: ['<span class="icon-21"></span>', '<span class="icon-20"></span>'],
			responsive: {
				0: {
					items: 1
				},
	...
	if ($('.testimonial-carousel').length) {
		...
		var itemCount = $carousel.find('.testimonial-item').length;
	...
	if ($('#cart-popup').length) {
		...
		$('.cart-toggler').on('click', function () {
			$('#cart-popup').addClass('popup-visible');
		});
		$(document).keydown(function (e) {
			if (e.keyCode === 27) {
				$('#cart-popup').removeClass('popup-visible');
			}
		});
		//Hide Popup
		$('.close-cart,.cart-popup .overlay-layer').on('click', function () {
			$('#cart-popup').removeClass('popup-visible');
		});
	}
	...
	if ($('.curved-circle').length) {
		$('.curved-circle').circleType({ position: 'absolute', dir: 1.18, radius: 86, forceHeight: true, forceWidth: true });
	}
