...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h5 class="mb-0 h6">{{trans('messages.attribute').' '.trans('messages.information')}}</h5>
</div>
        ...
        <div class="card-body p-0">
          ...
          <form class="p-4" action="{{ route('attributes.update', $attribute->id) }}" method="POST">
              ...
              <div class="form-group row">
                  <label class="col-sm-3 col-from-label" for="name">{{ trans('messages.name')}} <i class="las la-language text-danger" ></i></label>
                  <div class="col-sm-9">
                      <input type="text" placeholder="{{ trans('messages.name')}}" id="name" name="name" class="form-control" required value="{{ $attribute->getTranslation('name', $lang) }}">
                  </div>
              </div>
