...
function renderDesktopMenuItem($item) {
    ...
    if ($isMegamenu) {
        ...
        echo '<a href="' . $item->link . '" class="block px-4 py-8 text-sm font-bold text-gray-800 hover:text-blue-600 uppercase transition-colors">' . $item->label . ' <i class="fas fa-chevron-down ml-1 text-xs transition-transform group-hover:rotate-180"></i></a>';
        echo '<div class="absolute left-1/2 transform -translate-x-1/2 top-full pt-2 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50">';
        echo '<div class="bg-white rounded-2xl shadow-2xl border border-gray-100 p-8 min-w-[600px]">';
        echo '<div class="grid grid-cols-4 gap-6">';
        foreach ($item->child as $group) {
            echo '<div>';
            echo '<ul class="space-y-1 m-0 p-0 list-none">';
            echo '<li class="mb-3">';
            echo '<a href="' . $group->link . '" class="block font-semibold text-gray-900 hover:text-blue-600 transition-colors text-base">' . $group->label . '</a>';
            echo '</li>';
            if (isset($group->child) && count($group->child)) {
                foreach ($group->child as $sub) {
                    echo '<li>';
                    echo '<a href="' . $sub->link . '" class="block text-sm text-gray-600 hover:text-blue-600 hover:translate-x-1 transition-all py-2 px-4 rounded-lg hover:bg-blue-50">' . $sub->label . '</a>';
                    echo '</li>';
                }
            }
            echo '</ul>';
        ...
        echo '<li class="relative group">';
        echo '<a href="' . $item->link . '" class="block px-4 py-8 text-sm font-bold text-gray-800 hover:text-blue-600 uppercase transition-colors">' . $item->label . ' <i class="fas fa-chevron-down ml-1 text-xs transition-transform group-hover:rotate-180"></i></a>';
        echo '<ul class="absolute left-0 top-full pt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-100 p-4 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50 space-y-1">';
        ...
        echo '<li>';
        echo '<a href="' . $item->link . '" class="block px-4 py-8 text-sm font-bold text-gray-800 hover:text-blue-600 uppercase transition-colors">' . $item->label . '</a>';
        echo '</li>';
    }
}
...
function renderMobileMenuItem($item) {
    ...
    if ($isMegamenu) {
        echo '<li class="dropdown border-t border-gray-700">';
        echo '<a href="' . $item->link . '" class="block py-3 px-6 text-white text-sm font-medium uppercase hover:bg-gray-800 transition-colors">' . $item->label . '</a>';
        echo '<div class="dropdown-btn absolute right-2 top-3 w-8 h-8 flex items-center justify-center bg-gray-700 rounded cursor-pointer hover:bg-gray-600 transition-colors"><span class="fas fa-angle-down"></span></div>';
        ...
        echo '<li class="border-t border-gray-700">';
        echo '<a href="' . $item->link . '" class="block py-3 px-6 text-white text-sm font-medium uppercase hover:bg-gray-800 transition-colors">' . $item->label . '</a>';
        echo '</li>';
    }
}
